...
IF NOT EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND OBJECT_ID = OBJECT_ID('dbo.FTC_SP_csvSituacaoAlunosSimplificada'))
   exec('CREATE PROCEDURE [dbo].[FTC_SP_csvSituacaoAlunosSimplificada] AS BEGIN SET NOCOUNT OFF; END')
GO 
...
FROM LY_ALUNO A							-- PARA JOIN COM CURSO
LEFT JOIN VW_MATRICULA_E_PRE_MATRICULA PM
	ON PM.ALUNO = A.ALUNO
JOIN LY_PESSOA PES
...
WHERE C.ATIVO  = 'S' AND A.SIT_ALUNO = 'Ativo'  AND LD.VALOR > 0 AND LD.CODIGO_LANC IN ('MS','MS_EB') 
...
set @V_NOME_CONSULTA_DINAMICA ='Custom - Situação Alunos - Pré-matrícula e Matrícula - Simplificada'
...
VALUES(@V_NOME_CONSULTA_DINAMICA,'CSV','sp:FTC_SP_csvSituacaoAlunosSimplificada(@P_ANO@,@P_SEMESTRE@,@P_FACULDADE@,@P_TIPO_CURSO@,@P_CURSO@,@P_TURNO@,@P_CURRICULO@,@P_ANO_INGRESSO@,@P_SEM_INGRESSO@)')
...
INSERT INTO LY_CONSULTA_DINAMICA_PARAMETROS(ID_CONSULTA_DINAMICA,ORDEM,PARAMETRO,TIPO,DESCRICAO,OBRIGATORIO,SQL_TEXTO,COL_VALOR,COL_DESCR)
SELECT @V_CONSULTA_DINAMICA, 8, 'P_ANO_INGRESSO', 5, 'Ano de Ingresso', 'N', 'SELECT DISTINCT ANO AS CODIGO, (''ANO: '' + convert(varchar,ANO)) AS DESCRICAO FROM LY_PERIODO_LETIVO ORDER BY ANO DESC', 'CODIGO' COL_VALOR, 'DESCRICAO' COL_DESCR

INSERT INTO LY_CONSULTA_DINAMICA_PARAMETROS(ID_CONSULTA_DINAMICA,ORDEM,PARAMETRO,TIPO,DESCRICAO,OBRIGATORIO,SQL_TEXTO,COL_VALOR,COL_DESCR)
SELECT @V_CONSULTA_DINAMICA, 9, 'P_SEM_INGRESSO', 5, 'Semestre de Ingresso', 'N', 'SELECT DISTINCT PERIODO AS CODIGO, (''PERÍODO: '' + convert(varchar,PERIODO)) AS DESCRICAO FROM LY_PERIODO_LETIVO WHERE ANO = ISNULL(@p_ano_ingresso@,ANO) ORDER BY PERIODO', 'CODIGO' COL_VALOR, 'DESCRICAO' COL_DESCR
...
, 'Miguel' AUTOR
, '2017-02-01' DATA_CRIACAO
, 'ListaNG - Situação Alunos - Pré-matrícula e Matrícula - Simplificada' OBJETIVO
