...
IF NOT EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND OBJECT_ID = OBJECT_ID('dbo.FTC_Relat_Avaliacao_Institucional'))
   exec('CREATE PROCEDURE [dbo].[FTC_Relat_Avaliacao_Institucional] AS BEGIN SET NOCOUNT OFF; END')
GO 
  
ALTER PROCEDURE SituacaoFinanOK  
  @p_aluno T_CODIGO,   
...
-- [INÍCIO]   
  ...
  EXEC s_SituacaoFinanOK  @p_aluno, @pVerificaSoPrimParcPreMatr, @p_ano, @p_periodo, @p_SitFinanOk OUTPUT, @p_Msg_Erro OUTPUT, @p_TipoDivida, @p_AnoRef, @p_MesRef,@p_data_limite,@v_substitui output  
