...
IF NOT EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND OBJECT_ID = OBJECT_ID('dbo.FTC_Relat_Avaliacao_Mestrado'))
   exec('CREATE PROCEDURE [dbo].[FTC_Relat_Avaliacao_Mestrado] AS BEGIN SET NOCOUNT OFF; END')
GO 
  
ALTER PROCEDURE FTC_Relat_Avaliacao_Mestrado
...
*/    
-- Teste:    
--EXEC FTC_Relat_Avaliacao_Mestrado 'Av Docente M II', 'AVALDOCM2_20181', '04', NULL, NULL, NULL,NULL    
...
DECLARE    
 ...
 SET @p_avaliacaoq = 'Av Institucional I'  
 SET @p_aplicacao = 'AVAL_INST_1_2017'  
...
-- TABELA #TEMP_SUM_MEDIACURSOGERAL    
  ...
  INNER JOIN LY_CURSO AS C ON T.CURSO = C.CURSO    
  WHERE 1 = 1 AND C.CURSO = '3261'  
   AND T.FACULDADE = ISNULL(@p_unidade,T.FACULDADE)    
...
INNER JOIN LY_CURSO AS C ON C.CURSO = T.CURSO    
WHERE 1 = 1  AND C.CURSO = '3261'  
 AND AQ.TIPO_QUESTIONARIO = @p_avaliacaoq    
 AND R.APLICACAO = @p_aplicacao    
 --AND T.ANO = ISNULL(@p_ano,T.ANO)  
