...
IF NOT EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND OBJECT_ID = OBJECT_ID('dbo.FTC_SP_csvBolsasAlunoPeriodoLetivo'))
   exec('CREATE PROCEDURE [dbo].[FTC_SP_csvBolsasAlunoPeriodoLetivo] AS BEGIN SET NOCOUNT OFF; END')
GO 

ALTER PROCEDURE dbo.FTC_SP_csvBolsasAlunoPeriodoLetivo
(      
	@P_ANO VARCHAR(4)
...
--EXEC FTC_SP_csvBolsasAlunoPeriodoLetivo '2017', '2', '04', NULL, NULL
...
SET NOCOUNT ON  
  ...
  SELECT   
			  ...
			  B.DATA_ALT																AS [DATA_ALTERACAO], 
			  B.DATA_CANCEL																AS [DATA_CANCELAMENTO_BOLSA],
			  B.MOTIVO_CANCEL															AS [MOTIVO_CANCELAMENTO_BOLSA],
...
WHERE 1=1
AND (@P_ANO IS NOT NULL AND B.ANOINI = @P_ANO)
AND (@P_SEMESTRE IS NOT NULL AND B.MESINI BETWEEN (CASE WHEN @P_SEMESTRE IN ('1','11') THEN 1 WHEN @P_SEMESTRE = '0' THEN 1 ELSE 7 END) AND (CASE WHEN @P_SEMESTRE IN ('1','11')  THEN 6 WHEN @P_SEMESTRE = '0' THEN 12 ELSE 12 END))
AND (@P_FACULDADE IS NOT NULL AND C.FACULDADE = @P_FACULDADE) 
AND ((@P_TIPO_CURSO IS NOT NULL AND C.TIPO = @P_TIPO_CURSO) OR @P_TIPO_CURSO IS NULL OR @P_TIPO_CURSO = '')
AND ((@P_CURSO IS NOT NULL AND C.CURSO = @P_CURSO) OR @P_CURSO IS NULL OR @P_CURSO = '')
...
set @V_NOME_CONSULTA_DINAMICA ='Custom - Lista de Bolsas Lançadas no Período'
...
VALUES(@V_NOME_CONSULTA_DINAMICA,'CSV','sp:FTC_SP_csvBolsasAlunoPeriodoLetivo(@P_ANO@,@P_SEMESTRE@,@P_FACULDADE@,@P_TIPO_CURSO@,@P_CURSO@)')
...
SELECT @V_CONSULTA_DINAMICA, 1, 'P_ANO', 5, 'Ano', 'S', 'select distinct ano as codigo, ano as descricao from ly_turma order by ano', 'CODIGO' COL_VALOR, 'DESCRICAO' COL_DESCR

INSERT INTO LY_CONSULTA_DINAMICA_PARAMETROS(ID_CONSULTA_DINAMICA,ORDEM,PARAMETRO,TIPO,DESCRICAO,OBRIGATORIO,SQL_TEXTO,COL_VALOR,COL_DESCR)
SELECT @V_CONSULTA_DINAMICA, 2, 'P_SEMESTRE', 5, 'Semestre', 'S', 'SELECT DISTINCT SEMESTRE AS CODIGO, SEMESTRE AS DESCRICAO FROM LY_TURMA WHERE ANO = ISNULL(@p_ano@,ANO) ORDER BY SEMESTRE', 'CODIGO' COL_VALOR, 'DESCRICAO' COL_DESCR
...
where NOME = 'FTC_SP_csvBolsasAlunoPeriodoLetivo'
...
, 'Miguel' AUTOR
, '2018-01-20' DATA_CRIACAO
, 'ListaNG - Lista de Bolsas Lançadas no Período Letivo' OBJETIVO
